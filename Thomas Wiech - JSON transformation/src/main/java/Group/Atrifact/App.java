...
public class App {
	...
	public static void main(String[] args) throws Exception {
		...
		//Performing transformation details
		String finalENUM = finalOutputObject.toJSONString();
		String[] temp1 = finalENUM.split(":");
		finalENUM = "";
		for(int i = 0; i < temp1.length;i++) {
			finalENUM += temp1[i];
		}
		finalENUM = finalENUM.replaceAll("enum", "enum:");
		...
		for (int i = inputJsonArray.size(); i < inputFile.size(); i++) {
			...
			structs.add(finalOutput.toJSONString().replaceAll("default\":", "default\"").replace("default\"{","default\"{\n"));
		...
		//performing detailed transformation
		String strut = "";
		for (int i = 0; i < structs.size(); i++) {
			strut += structs.get(i).substring(1, structs.get(i).length() - 1).replace(",", "")+"\n";
			System.out.println(structs.get(i).substring(1, structs.get(i).length() - 1).replace(",", "")+"\n");
